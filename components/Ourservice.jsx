...
export default function Ourservice() {
  ...
  useEffect(() => {
    // Set navigation ready after refs are attached
      ...
      <div className="flex items-center justify-center px-4 py-8 md:py-12 lg:py-16">
        <div className="max-w-4xl text-center">
          ...
          <p className="sm:text-md mt-4 text-gray-600 md:text-lg">
            ...
            integrated B2B & B2C ecosystem featuring hyperlocal Byyizzy-powered stores and advanced
            technology.
          </p>
        </div>
      </div>
                  ...
                  <SwiperSlide key={index}>
                    {({ isActive }) => (
                      <Link href={service.href}>
                        ...
                        >
                          <div
                            className={`flex h-20 w-20 items-center justify-center rounded-lg transition-all duration-500 ${isActive ? "bg-white" : "bg-blue-100"}`}
                          >
                            <Image src={service.icon} alt={service.text} width={64} height={64} />
                          </div>
                          <h4
                            className={`mt-5 mb-2 text-lg font-semibold transition-colors duration-500 ${isActive ? "text-white" : "text-gray-900"}`}
                          >
                            {service.text}
                          </h4>
                          <div className="mt-4 flex items-center text-sm font-medium transition-colors duration-300">
                            <span
                              className={`flex cursor-pointer items-center gap-2 ${isActive ? "text-white hover:text-gray-200" : "text-blue-600 hover:text-blue-800"}`}
                            >
